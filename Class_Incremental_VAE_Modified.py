"""
Class Incremental Learning with Conditional VAE (v10.6: Prof Feedback Applied).
...
class ExperimentConfig:
    ...
    save_model_each_step: bool = True  # <-- 추가: 스텝마다 모델 저장할지 여부
...
class CIL_Trainer:
    ...
    def _calculate_batch_loss_corrected(self, logits, x_all, y_all, mu, lv, beta, lam_eff):
        ...
        if self.use_contrastive and self.seen:
            # 이미 proj(mu)를 사용하고 있고, 여기서 normalize
    ...
    def _log_batch_to_wandb(self, total, bce, kl, con_loss_val, batch_p_pos, x_all, y_all, lam_eff):
        wb_payload = {
            "Train/total": total.item(),
            "Train/BCE": bce.item(),
            "Train/KL": kl.item(),
            "Train/Recon": bce.item(),  # recon loss alias
        }
    ...
    def _log_epoch_to_wandb(self, epoch_metrics: dict, eval_metrics: dict):
        ...
        if CFG.save_sample_grid:
            with torch.no_grad():
                # Real Grid: 현재 스텝 새 클래스만
                X_real_curr, _ = generate_eval_samples(self.model, [self.current_new_c], 8, self.mu_proto_bank)
                ...
                # Fake Grid: 지금까지 본 전체 클래스(이전 + 현재)
                classes_to_fake = self.seen + [self.current_new_c]
                _, X_fake_all = generate_eval_samples(self.model, classes_to_fake, 8, self.mu_proto_bank)
                if X_fake_all.numel() > 0:
                    wb_payload["Samples/Fake_Grid"] = wandb.Image(
                        make_grid(X_fake_all[:64], nrow=8, padding=2),
                        caption=f"Fake (Seen Classes {classes_to_fake})"
                    )
    ...
    def _save_local_artifacts(self, step, eval_metrics: dict):
        ...
        if CFG.save_sample_grid:
            # Real: 현재 스텝 새 클래스만 / Fake: 지금까지 본 모든 클래스
            X_real_small, _ = generate_eval_samples(self.model, [self.current_new_c], 16, self.mu_proto_bank)
            _, X_fake_small = generate_eval_samples(self.model, self.seen, 16, self.mu_proto_bank)
            save_image_grid(
    ...
    def run_cil(self, stream_classes):
        for step, new_c in enumerate(stream_classes, 1):
            ...
            for ep in range(CFG.epochs_cil):
                ...
                # λ를 hidden dimension(z_dim)으로 스케일링
                raw_lam = lambda_con_at_epoch(ep, CFG.lambda_warmup_ep, CFG.lambda_con_max) if self.use_contrastive else 0.0
                lam_eff = raw_lam / float(CFG.z_dim) if self.use_contrastive else 0.0
                ...
                for x_new, y_new in dl_new:
                    ...
                    total, bce, kl, con_loss_val, batch_p_pos = self._calculate_batch_loss_corrected(
                    ...
                    if self.wb_run: 
                        self._log_batch_to_wandb(total, bce, kl, con_loss_val, batch_p_pos, x_all, y_all, lam_eff)
            ...
            # 스텝 종료 후: 현재 클래스 officially seen에 추가하고, 아티팩트 저장
...
def main():
    ...
    wbA.log(
        {
            "Samples/Fake_Grid": wandb.Image(
                make_grid(generate_eval_samples(modelA, base_classes, 16, trainerA.mu_proto_bank)[1], nrow=8, padding=2),
                caption="Initial Base Generator"
            )
        },
        step=0
    )
    ...
    wbB.log(
        {
            "Samples/Fake_Grid": wandb.Image(
                make_grid(generate_eval_samples(modelB, base_classes, 16, trainerB.mu_proto_bank)[1], nrow=8, padding=2),
                caption="Initial Base Generator"
            )
        },
        step=0
